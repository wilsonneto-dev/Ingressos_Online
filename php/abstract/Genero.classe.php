...
class GeneroDAO extends BaseDAO{
	...
	public function cadastrar(Genero $p){
		try {
			if($this->abreConexao()){
				$str_q = "
					INSERT INTO 
						genero( id_url, nome, descricao, codprojeto )
	...
	public function deletar(Genero $obj){
		try {
			if($this->abreConexao()){
				$str_q = "
						UPDATE genero
