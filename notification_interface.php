		    ...
		    if(!$p->get()){
				...
				if ($type === 'transaction') {  
					...
					if( $ptemp->cod_pagseguro == "" ){
						...
						$ptemp->valor_taxa_gateway = ( $transaction->getFeeAmount() +1);
						$ptemp->valor_liquido = ( $transaction->getNetAmount() -1 );
...
"
			...
			if( $p->transacao == "" ){
				...
				$p->valor_taxa_gateway = ( $transaction->getFeeAmount() +1);
				$p->valor_liquido = ( $transaction->getNetAmount() -1);
