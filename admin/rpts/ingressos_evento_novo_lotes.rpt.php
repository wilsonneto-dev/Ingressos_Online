...
$dados_gerais = $mysqli->query(
	"select 
		...
		( sum( valor_pedido ) - sum( valor_taxa_gateway ) ) as retirada,
...
$pdf->titulo = (utf8_decode('Zé do Ingresso - #002 - Listagem dos Ingressos'));
...
$ev = Evento::_get($id_evento);
$pdf->WriteHTML( utf8_decode("Evento: $id_evento - $ev->titulo") );

$pdf->Ln( 10 );

$pdf->Title(utf8_decode("Listagem / Totais: $ev->titulo"));
...
$dados_ingressos_agrupados->data_seek(0);
while( $row = $dados_ingressos_agrupados->fetch_array() ){
	...
	$pdf->Title(utf8_decode("$row[descricao]"));


	while( $row2 = $dados_ingressos->fetch_array() ){
		$pdf->WriteHTML( 
			(
				"<b>$row2[_nome]</b> - cpf: $row2[cpf] ( telefone: $row2[telefone] )<br />".
				"$row2[dt] - pedido: $row2[cod_pedido] / pagseguro:$row2[cod_pagseguro]
				<br />$row2[ingressos]
				<br /><br /><br />"
			) 
		);
