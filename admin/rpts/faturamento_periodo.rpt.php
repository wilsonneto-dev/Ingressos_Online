...
$dados_gerais = $mysqli->query(
	...
	from temp_pedido ped
	WHERE
		ped.ativo = 1
		and ped.cod_status in ( 3, 4 )
		and ped.data_cadastro > str_to_date( '$data_range_inicial','%d/%m/%Y' )
		and ped.data_cadastro < date_add( str_to_date( '$data_range_final','%d/%m/%Y' ), interval 1 day )
		and '$cod_evento' in ( ped.evento, 'Todos' )
");
...
$lista_dias = $mysqli->query("
	...
	left join temp_pedido ped 
		on dias.dia = date(ped.data_cadastro)
		and ped.ativo = 1
		and ped.cod_status in ( 3, 4 )
		and ped.data_cadastro > str_to_date( '$data_range_inicial','%d/%m/%Y' )
		and ped.data_cadastro < date_add( str_to_date( '$data_range_final','%d/%m/%Y' ), interval 1 day )
...
$pdf->WriteHTML(
	"Evento: $cod_evento<br />Data: de $data_range_inicial a $data_range_final"
);
