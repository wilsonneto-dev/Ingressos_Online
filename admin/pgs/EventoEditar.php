...
if( isset( $_GET["id"] ) ) {
	...
	if($p->get()){
		...
		$tags_string = EventoTag::getByEvento( $p->id );		
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			*/
			...
			if( $p->atualizar() ){
				LogAdmin::_salvar( "Evento Atualizado", "Evento" , $admin->id , json_encode( $clone ), json_encode( $p ) );
				addOnloadScript("message('Atualizado com sucesso.','sucess');");
			} else {
				LogAdmin::_salvar( "Erro ao atualizar Evento", "Evento" , $admin->id , "", json_encode( $clone ), json_encode( $p ) );
				addOnloadScript("message('Ocorreu um erro ao atualizar.','error');");
			}

			// tags
			EventoTag::deletarByEvento( $p->id );
			$tags = $_POST["tags"];
...
$page->form_fields = array(
	...
	array( "value" => date_format( $p->data_encerrar_vendas , 'd/m/Y' ), "type" => "data", "name" => "data_encerrar_vendas", "label" => "Data Encerrar ( as vendas ser&atilde;o encerradas ao final do dia informado)", "required" => true ),
	array( "value" => date_format( $p->data , 'd/m/Y' ), "type" => "data", "name" => "data", "label" => "Data do Evento ( Para ordenação, primeiro dia )", "required" => true ),
	array( "value" => date_format( $p->data_final , 'd/m/Y' ), "type" => "data", "name" => "data_final", "label" => "Data Final do Evento ( Sair da listagem, último dia )", "required" => true ),
	...
	array( "name" => "imagem", "label" => "Imagem ".EVENTO_IMAGEM_WIDTH."x".EVENTO_IMAGEM_HEIGHT, "type" => "file" ),
	...
	array( "name" => "imagem_capa", "label" => "Imagem Capa ".EVENTO_CAPA_WIDTH."x".EVENTO_CAPA_HEIGHT, "type" => "file" ),
	...
	array( "value" => $p->link_facebook, "name" => "link_facebook", "label" => "Link Facebook", "icon" => "fa-chain" ),

	/* visivel */
	array( "value" => $p->visivel, "name" => "visivel", "label" => "Vis&iacute;vel ( acess&iacute;vel )", "type" => "checkbox" ),
	array( "value" => $p->oculto, "name" => "oculto", "label" => "Oculto ( n&atilde;o mostrar na lista nem na busca )", "type" => "checkbox" ),
	array( "value" => $p->venda_suspensa, "name" => "venda_suspensa", "label" => "Vendas Suspensas ( desativar vendas )", "type" => "checkbox" ),
	
	array( "value" => $p->acertado, "name" => "acertado", "label" => "Acerto j&aacute; feito", "type" => "checkbox" ),
