...
/* sqls */
$r_box_clientes_novos = new Repeater();
$r_box_clientes_novos->campos = "clientes;novos;recorrentes;recorrencia";
$r_box_clientes_novos->sql = "
    select 
        clientes,
        novos,
        ( clientes - novos ) as recorrentes,
        cast( ( ( clientes - novos ) / clientes * 100 ) as decimal(10,1) ) as recorrencia
    from (
        select 
            count( distinct ped.cod_usuario ) as clientes,
            count( distinct (
                case 
                    when ped.cod_usuario not in ( 
                        select 
                            ped.cod_usuario
                        from pedido as ped
                        WHERE
...
$r_box_clientes_novos->txtItem = '
    <div class="col-lg-3 col-xs-6">
        ...
        <div class="small-box bg-aqua">
            ...
            <a href="/admin/?pg=Clientes" class="small-box-footer">
...
$r_receita_semanal->sql = "
    ...
    where
        ...
        and yearweek( dia ) like concat( '', year( CURDATE() ), '%' )
    group by 
        yearweek( dia )
    order by
        dia
...
$r_campanhas = new Repeater();
$r_campanhas->campos = "id;data;evento;qtd_pedidos;valor;ingressos;ingressos;liquido_pedido;liquido;ref";
$r_campanhas->sql = "
    select 
        ped.ref,
        ev.id,
    ...
    WHERE
        ...
        and ev.data >= date_add( str_to_date( '".$data_final->format("d/m/Y")."','%d/%m/%Y' ), interval 1 day )
        AND ev.ativo = 1 
        AND ped.ref <> ''
        and ped.data > str_to_date( '".$data_inicial->format("d/m/Y")."','%d/%m/%Y' )
        and ped.data < date_add( str_to_date( '".$data_final->format("d/m/Y")."','%d/%m/%Y' ), interval 1 day )
...
if($r_campanhas->contador > 0){
    $page->html_content .= '
        <div class="box-header">
            <h3 class="box-title">Campanhas de Marketing</h3>
        </div>
        <div class="box-body table-responsive">
