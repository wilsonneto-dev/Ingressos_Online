...
if( isset($_GET["id"]) ){
	...
	if($p->get()){
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			$p->localizacao_latitude = $localizacao_latitude;
			$p->localizacao_longitude = $localizacao_longitude;
			...
			if($p->atualizar()){
				LogAdmin::_salvar( "Local Editado", "Local" , $admin->id, json_encode( $clone ), json_encode( $p ) );
				addOnloadScript("message('Atualizado com sucesso.','sucess');");
			} else {
				LogAdmin::_salvar( "Erro ao editar Local", "Local" , $admin->id, json_encode( $clone ), json_encode( $p ) );
...
$page->form_fields = array(
	...
	array( "name" => "capa", "label" => "Imagem (".LOCAL_CAPA_WIDTH."x".LOCAL_CAPA_HEIGHT.")", "type" => "file" ),
	array( "label" => "Logo Atual", "type" => "image-view", "value" => ( "/" . $p->logo ) ),
	array( "name" => "logo", "label" => "Logo (".LOCAL_LOGO_WIDTH."x".LOCAL_LOGO_HEIGHT.")", "type" => "file" ),
